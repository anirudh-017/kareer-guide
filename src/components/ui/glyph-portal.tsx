  ...
  useLayoutEffect(() => {
    ...
    const readInk = () => {
      ...
      const advances = Array.from(
        { length: text.length },
        (_, i) => context.measureText(text.slice(0, i)).width,
      );
      ...
      target =
        ...
        [...candidates].sort(
          (a, b) => b.radius - a.radius || Math.abs(a.x - center.x) - Math.abs(b.x - center.x),
        )[0] ??
        null;
      return true;
    };
    ...
    const select = (next: Ink | null) => {
      ...
      for (const option of Array.from(picker.options))
        option.disabled =
          option.value === "" ||
          !candidates.some((candidate) => candidate.index === Number(option.value));
    ...
    const layout = () => {
      ...
      if (fontDirty) {
        ready = readInk();
        // Local change to the vendored component: only stop re-measuring once a
        // measurement actually succeeded. Clearing the flag unconditionally
        // meant one zero-width canvas metric on a cold first frame left the
        // hero permanently blank, with no resize able to recover it.
        fontDirty = !ready;
      }
      if (!ready) return;
      ...
      section.dataset["gpMotion"] =
        !motion.matches && browserFrameSeen && !stalled && target ? "on" : "off";
    };
        ...
        event.key === "Home"
          ...
          : event.key === "End"
            ...
            : (current +
              ...
              candidates.length;
      buttons
        .find((button) => Number(button.dataset["gpLetter"]) === candidates[index]!.index)
